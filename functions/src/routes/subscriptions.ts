...
export const subscriptionRoutes = (app: express.Router) => {
     ...
     * Search & return users: 
     */
    app.post("/subscriptions/next", validateKey, async (req: express.Request, res: express.Response) => {
        functions.logger.debug(" ✅ [ORDERS]: Subscriptions Paginate Next Start Route");
        let status = 200,
            text = " 🎉 [SUCCESS]: Subscriptions sucessfully fetched",
            result: SubscriptionAgreement[] = [],
            size = 0,
            ok = true;
        ...
        try {
            ...
            const response = await getPaginatedCollections(merchant_uuid, "subscriptions", start);
            if (response?.data?.collection && response.status < 300) {
                result = response?.data?.collection;
                size = response?.data?.size ? response?.data?.size : 1;
            }
            ...
            text = " 🚨 [ERROR]: Likely a problem fetching a subscriptions";
            status = 500;
            ok = false;
            functions.logger.error(text);
            throw new Error(text);
